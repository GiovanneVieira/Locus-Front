      ...
      <main className="mx-auto max-w-5xl px-6 py-12">
          ...
          <div className="relative flex flex-col items-start gap-6 sm:flex-row sm:items-center">
            <div className="relative">
              {user.avatarUrl ? (
                ...
                <div className="flex size-20 items-center justify-center rounded-2xl border border-white/10 bg-white/10 text-xl font-semibold">
                  {getInitials(user.name)}
                </div>
              )}
              <div className="absolute -right-2 -bottom-2 rounded-full border border-white/10 bg-background p-1.5">
                {host ? (
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3">
                ...
                {host ? (
                  ...
                  <span className="inline-flex items-center gap-1 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                    Usuário comum
                  </span>
                )}
              </div>
        ...
        <section className="mt-8 grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
          <div className="glass-card p-6">
            <div className="mb-5 flex items-center justify-between gap-4">
              ...
              {!isEditing ? (
                <Button
                  variant="outline"
                  className="rounded-full border-white/15 bg-white/5 px-4"
              ...
              {isEditing ? (
                <div className="flex flex-wrap gap-3">
                  ...
                  <Button
                    type="button"
                    variant="outline"
                    className="h-11 rounded-xl border-white/15 bg-white/5 px-6"
                    onClick={() => {
                      form.reset()
                      setIsEditing(false)
                      setFeedback(null)
          ...
          <div className="glass-card flex flex-col gap-5 p-6">
            <div>
              <h2 className="text-lg font-semibold">Seus endereços</h2>
              <p className="text-xs text-muted-foreground">
            ...
            {!host ? (
              <div className="rounded-2xl border border-dashed border-white/10 bg-white/5 p-4 text-sm text-muted-foreground">
                Você ainda não é anfitrião. Torne-se anfitrião para publicar endereços.
              </div>
